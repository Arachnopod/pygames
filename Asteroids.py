...
# 
# I am teaching my 12 yr old daughter how to program in Python
# so I am shadowing the course with her.
#
# Author: John Liu
# Date: 2014-May-24
...
class Ship:
    ...
    def distance(self,obj_pos):
        return math.sqrt((self.pos[0]-obj_pos[0])**2 + (self.pos[1]-obj_pos[1])**2)
...
class Sprite:
    ...
    def collide(self,other_object):
        ...
        distance = (self.pos[0]-other_obj_pos[0])**2 + (self.pos[1]-other_obj_pos[1])**2
        return distance < (self.radius + other_object.get_radius())**2
...
# group group collide between rocks and missiles
...
def draw(canvas):
    global time, started, lives, score, rock_group, missile_group, explosion_group, soundtrack
    ...
    # update rocks & missiles & explosions
    ...
    # check for rocks & missile collisions
...
# helper function to update rocks
...
def rock_spawner():
    ...
    rock_pos = my_ship.get_position()
    if started:
        while my_ship.distance(rock_pos) < 200:
            rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]

        multiplier = score // 100    
        rock_vel = [(random.random() * 1.2 - .6)*multiplier, (random.random() * 1.2 - .6)*multiplier]
...
frame.add_label('Left Arrow: turn left')
frame.add_label('Right Arrow: turn right')
frame.add_label('Up Arrow: thrust')
frame.add_label('Space Bar: shoot')
frame.add_label('z: hyperspace')
